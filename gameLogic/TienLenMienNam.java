...
public class TienLenMienNam {
	...
	private CardofTienLen minCard = new CardofTienLen();// đã thêm-là lá bài bé nhất, 4 players -> là 3 bích
	...
	public void newMotherPack() {
		...
		for (CardofTienLen.Value value : CardofTienLen.Value.values()) {
			for (CardofTienLen.Type type : CardofTienLen.Type.values()) {
				motherPack.addCard(new CardofTienLen(value, type));
			}
		}
...
//		motherPack.printDeck();
	...
	public boolean checkDoi(CardofTienLen card1, CardofTienLen card2) {
	...
	public void deselectCard(CardofTienLen card) {
		selectionCard.getCards().remove(card);
	}

	public void setSelectionCard(CardofTienLen card) {
